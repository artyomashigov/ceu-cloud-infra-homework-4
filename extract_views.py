...
# See https://wikimedia.org/api/rest_v1/#/Edited%20pages%20data/get_metrics_edited_pages_top_by_views__project___editor_type___page_type___year___month___day_
url = f"https://wikimedia.org/api/rest_v1/metrics/pageviews/top/en.wikipedia.org/all-access/{date.strftime('%Y/%m/%d')}"

headers = {"User-Agent": "ashigov_artyom@student.ceu.edu"}
...
########
#
# Save the contents of `wiki_response_body` to file called `raw-views-YYYY-MM-DD.txt` into the folder
...
s3 = boto3.client("s3")
S3_WIKI_BUCKET = "ceu-artyom-wikidata"
...
# Improved check for bucket existence
try:
    s3.head_bucket(Bucket=S3_WIKI_BUCKET)
except s3.exceptions.NoSuchBucket:
    raise AssertionError(f"The bucket {S3_WIKI_BUCKET} does not exist")
...
s3.upload_file(
    json_lines_file, S3_WIKI_BUCKET, f"datalake/bronze_views/{json_lines_filename}"
)
print(
    f"Uploaded JSON lines to s3://{S3_WIKI_BUCKET}/datalake/bronze_views/{json_lines_filename}"
)
# %%
